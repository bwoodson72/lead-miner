...
export async function POST(request: NextRequest) {
  ...
  try {
    const response = await fetch(`${API_URL}/api/automation/run`, { method: "POST", headers: { Authorization: `Bearer ${secret}`, "Content-Type": "application/json" }, body: await request.text() });
