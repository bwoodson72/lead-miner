...
type Settings = {
  ...
  approvalMode: "manual" | "shadow" | "auto_safe";
  researchModel: string;
  outreachModel: string;
  researchBatchSize: number;
  minAutoApprovePriority: number;
  minAutoApproveConfidence: number;
  minProblemConfidence: number;
...
export default function SettingsPage() {
  ...
  const numberField = (key: keyof Settings, label: string, step = 1) => (
    <label className="block text-sm text-zinc-300">{label}<input type="number" step={step} value={settings[key] as number} onChange={(e) => setSettings({ ...settings, [key]: Number(e.target.value) })} className="mt-1 w-full rounded border border-zinc-700 bg-zinc-950 px-3 py-2 text-white" /></label>
  );
  ...
  return <main className="min-h-screen bg-zinc-950 text-white"><div className="mx-auto max-w-4xl px-4 py-8">
    ...
    <section className="mb-6 rounded-xl border border-zinc-800 bg-zinc-900 p-5"><h2 className="mb-4 font-semibold">AI</h2><div className="grid gap-4 md:grid-cols-2">
      ...
      <label className="block text-sm">Outreach model<input value={settings.outreachModel} onChange={(e) => setSettings({ ...settings, outreachModel: e.target.value })} className="mt-1 w-full rounded border border-zinc-700 bg-zinc-950 px-3 py-2" /></label>
      {numberField("minProblemConfidence", "Minimum problem confidence", 0.01)}
      {numberField("minAutoApproveConfidence", "Minimum auto-approval confidence", 0.01)}
      {numberField("minAutoApprovePriority", "Minimum auto-approval priority")}
    </div></section>
