...
type SiteCoverage = { mode:string; sitemapUrlsFound:number; representativePagesAttempted:number; representativePagesFetched:number; serviceUrlsObserved:number; locationUrlsObserved:number; aboutUrlsObserved:number; architectureEvidenceComplete:boolean; warning:string };
...
export default function LeadDetailPage({ params }:{ params:Promise<{id:string}> }) {
  ...
  const isV6=lead.researchVersion==="lead-research-v6"&&Boolean(assetAssessment);
  ...
  return <main className="min-h-screen bg-zinc-950 text-white"><div className="mx-auto max-w-6xl px-4 py-8">
    ...
    <div className="mb-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
      ...
      <div className="rounded-lg border border-zinc-800 bg-zinc-900 p-4"><div className="text-xs uppercase text-zinc-500">{isV6?"Asset strength":"AI priority"}</div><div className={`mt-1 text-xl font-bold capitalize ${isV6?ratingClass(assetAssessment?.assetStrength??"unknown"):""}`}>{isV6?label(assetAssessment?.assetStrength):lead.priorityScore??"—"}</div></div>
